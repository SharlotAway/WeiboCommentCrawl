...
def get_buildComments_level_two_response(uid, mid, cookie, the_first=True, max_id=None):
    ...
    response = requests.get(
        "https://weibo.com/ajax/statuses/buildComments", params=params, headers=headers, timeout=10
    )
    return response
...
def get_all_level_two(uid, mid, cookie, max_times=1500):
    # 初始化
    max_id = ""
    data_lst = []
...
# def get_all_level_two(uid, mid, cookie, max_times=100):
#     # 初始化
#     max_id = ""
#     data_lst = []
#     max_times = max_times  # 最多只有15页
...
#                     )
#             except Exception as e:
#                 raise RuntimeError(f"Error at fetching data for page {current_times}: {e}")
...
#                 data = pd.concat(data_lst).reset_index(drop=True)
#             except Exception as e:
#                 raise RuntimeError(f"Error when concatenating data: {e}")
            
#             try:
#                 data = process_data(data)
#             except Exception as e:
#                 raise RuntimeError(f"Error when processing data: {e}")
            
#             try:
#                 data.insert(0, "main_body_uid", uid)
#                 data.insert(0, "comments_level_1_mid", mid)
#             except Exception as e:
#                 raise RuntimeError(f"Error when inserting uid and mid into data: {e}")
...
#     except Exception as e:
#         raise ValueError(f"解析页面失败，请检查你的cookie是否正确！ 具体错误信息: {e}")
